...
```{r}
#commented out bc runtime is long...
#summary report of expected winnings, proportions of wins, and playing time with each type of game
# one_ball_report<-function(){
#   red_expwins = replicate(5, exp_bet_red()[1])
#   no_expwins = replicate(5, exp_bet_no()[1])
#   mart_expwins = replicate(5, exp_bet_martingale()[1])
#   leb_expwins = replicate(5, exp_bet_labouchere()[1])
#   red_propwins = replicate(5, prop_bet_red())
#   no_propwins = replicate(5, prop_bet_no())
#   mart_propwins = replicate(5, prop_bet_martingale())
#   leb_propwins = replicate(5, prop_bet_labouchere())
#   red_playtime <- 1
#   no_playtime <- 1
#   mart_playtime = replicate(5, bet_martingale()[2])
#   leb_playtime = replicate(5, bet_labouchere()[2])
#   return (list(min(red_expwins),max(red_expwins),
#                min(no_expwins),max(no_expwins),
#                min(mart_expwins),max(mart_expwins),
#                min(leb_expwins),max(leb_expwins),
#                min(red_propwins),max(red_propwins),
#                min(no_propwins),max(no_propwins),
#                min(mart_propwins),max(mart_propwins),
#                min(leb_propwins),max(leb_propwins),
#                min(red_playtime),max(red_playtime),
#                min(no_playtime),max(no_playtime),
#                min(mart_playtime),max(mart_playtime),
#                min(leb_playtime),max(leb_playtime)))
# }
# 
# one_ball_report()
```
...
```{r}
#commented out bc runtime is long...
#calculate mean and std dev of above results
# red_wins <- function() {
...
# 
# red_prop <- function() {
#   g1 <- replicate(100000, prop_bet_red())
#   
#   return(list(mean(g1), sd(g1)))
# }
# 
